...
export default function AIFoodAssistant() {
  ...
  return createPortal(
    <>
      ...
      <motion.button type="button" onClick={() => setOpen((current) => !current)} whileHover={{ scale: 1.06, y: -2 }} whileTap={{ scale: 0.94 }} className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] right-24 z-[91] flex h-14 w-14 items-center justify-center rounded-full border border-white/20 bg-gradient-to-br from-orange-500 to-rose-500 text-white shadow-[0_20px_55px_rgba(249,115,22,0.38)] focus:outline-none focus:ring-4 focus:ring-orange-500/25 sm:bottom-8 sm:right-28 sm:h-16 sm:w-16" aria-label={open ? 'Close AI Food Assistant' : 'Open AI Food Assistant'} aria-expanded={open}><AnimatePresence mode="wait">{open ? <motion.span key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }}><X className="h-6 w-6" /></motion.span> : <motion.span key="bot" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} className="relative"><Bot className="h-7 w-7" /><Sparkles className="absolute -right-2 -top-2 h-3.5 w-3.5" /></motion.span>}</AnimatePresence></motion.button>
    </>,
    document.body
  );
}
