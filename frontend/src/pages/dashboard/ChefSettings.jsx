...
import { deactivateChef, saveChefNotificationPreferences } from '../../services/chefSettingsApi';
...
const preferenceOptions = [
  { key: 'orderNotifications', label: 'Order Notifications', description: 'New orders and order status activity.', icon: Bell },
  { key: 'newReviewNotifications', label: 'New Review Notifications', description: 'Customer ratings and new reviews.', icon: Star },
  { key: 'messages', label: 'Messages', description: 'New customer and support conversations.', icon: MessageCircle },
  { key: 'promotions', label: 'Promotions', description: 'HomeBite campaigns and promotional opportunities.', icon: Sparkles },
  { key: 'systemUpdates', label: 'System Updates', description: 'Security, platform, and service announcements.', icon: Laptop }
];
...
export default function ChefSettings() {
  ...
  async function submitPreferences(values) {
    try {
      await saveChefNotificationPreferences(email, values);
      await refreshDbUser().catch(() => {});
      toast.success('Notification preferences saved.');
    } catch (error) {
      toast.error(error.response?.data?.message || 'Unable to save notification preferences.');
    }
  }
  ...
  async function confirmDeactivation() {
    ...
    setDeactivateError('');
    try {
      await deactivateChef(email);
      await refreshDbUser();
      toast.success('Chef profile deactivated. Your account is now a customer account.');
      setConfirmOpen(false);
      navigate('/', { replace: true });
    } catch (error) {
      const message = error.response?.data?.message || 'Unable to leave as chef.';
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <DashboardHeader title="Chef Settings" description="Manage security, notifications, appearance, language, and your chef account status." />
      ...
      {usesPassword ? (
        <SettingsCard icon={KeyRound} title="Change Password" description="Choose a strong password you do not use elsewhere." delay={0.03}>
          <form onSubmit={passwordForm.handleSubmit(submitPassword)} noValidate className="space-y-5">
            ...
            <div className="flex justify-end"><motion.button whileHover={!passwordForm.formState.isSubmitting ? { y: -2 } : {}} whileTap={!passwordForm.formState.isSubmitting ? { scale: 0.98 } : {}} type="submit" disabled={passwordForm.formState.isSubmitting} className="inline-flex min-w-44 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-orange-500 to-rose-500 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-orange-500/20 disabled:cursor-not-allowed disabled:opacity-50">{passwordForm.formState.isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <LockKeyhole className="h-4 w-4" />}{passwordForm.formState.isSubmitting ? 'Updating...' : 'Update Password'}</motion.button></div>
          </form>
        </SettingsCard>
      ) : usesGoogle ? (
      ...
      <SettingsCard icon={Bell} title="Notification Preferences" description="Choose which HomeBite updates you want to receive." delay={0.06}>
        <form onSubmit={preferenceForm.handleSubmit(submitPreferences)} className="space-y-4">{preferenceOptions.map((option) => <PreferenceRow key={option.key} option={option} checked={Boolean(preferences[option.key])} onChange={(checked) => preferenceForm.setValue(option.key, checked, { shouldDirty: true })} />)}<div className="flex justify-end pt-2"><motion.button whileHover={!preferenceForm.formState.isSubmitting ? { y: -2 } : {}} whileTap={!preferenceForm.formState.isSubmitting ? { scale: 0.98 } : {}} type="submit" disabled={preferenceForm.formState.isSubmitting} className="inline-flex min-w-40 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-orange-500 to-rose-500 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-orange-500/20 disabled:opacity-50">{preferenceForm.formState.isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{preferenceForm.formState.isSubmitting ? 'Saving...' : 'Save Preferences'}</motion.button></div></form>
      </SettingsCard>

      <SettingsCard icon={Sun} title="Appearance" description="Choose how HomeBite looks across this browser." delay={0.09}><fieldset><legend className="sr-only">Theme preference</legend><div className="grid gap-3 sm:grid-cols-3">{[{ value: 'light', label: 'Light', icon: Sun }, { value: 'dark', label: 'Dark', icon: Moon }, { value: 'system', label: 'System', icon: Laptop }].map((option) => <ThemeOption key={option.value} option={option} selected={themePreference === option.value} onClick={() => setTheme(option.value)} />)}</div></fieldset></SettingsCard>

      <SettingsCard icon={Languages} title="Language" description="Language selection is a preview; translations are not enabled yet." delay={0.12}><label htmlFor="chef-language" className="block max-w-md text-sm font-semibold text-[var(--text-secondary)]">Display Language<select id="chef-language" value={language} onChange={(event) => setLanguage(event.target.value)} className={inputClass}><option value="en">English</option><option value="bn">বাংলা</option><option value="it">Italiano</option></select><span className="mt-2 block text-xs font-normal text-[var(--text-muted)]">Placeholder only—page content remains in English.</span></label></SettingsCard>

      <SettingsCard icon={ShieldOff} title="Leave as Chef" description="Convert this chef profile back into a regular customer account." delay={0.15} danger><div className="flex flex-col gap-5 rounded-2xl border border-red-500/20 bg-red-500/5 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[var(--text-primary)]">Deactivate Chef Profile</p><p className="mt-2 max-w-2xl text-xs leading-6 text-[var(--text-muted)]">Your user account and historical orders, payments, reviews, notifications, revenue, and chats will remain. Your foods will become unavailable.</p></div><button type="button" onClick={() => { setDeactivateError(''); setConfirmOpen(true); }} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-red-500/30 px-5 py-3 text-sm font-semibold text-red-500 transition hover:bg-red-500/10 focus:outline-none focus:ring-2 focus:ring-red-500/20"><LogOut className="h-4 w-4" /> Leave as Chef</button></div></SettingsCard>

      <SettingsCard icon={Laptop} title="Logout All Devices" description="End active HomeBite sessions on every device." delay={0.18}><div className="flex flex-col gap-4 rounded-2xl border border-[var(--border)] bg-[var(--bg-muted)] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[var(--text-primary)]">Remote session management</p><p className="mt-1 text-xs text-[var(--text-muted)]">This security feature is not available yet.</p></div><button type="button" disabled className="rounded-full border border-[var(--border)] px-5 py-3 text-sm font-semibold text-[var(--text-muted)] opacity-60">Coming Soon</button></div></SettingsCard>

      <AnimatePresence>{confirmOpen && <DeactivateModal busy={deactivating} error={deactivateError} onClose={() => !deactivating && setConfirmOpen(false)} onConfirm={confirmDeactivation} />}</AnimatePresence>
    </div>
  );
}
...
function PasswordField({ label, name, register, error, rules }) { return <label htmlFor={name} className="block text-sm font-semibold text-[var(--text-secondary)]">{label}<input id={name} type="password" autoComplete={name === 'currentPassword' ? 'current-password' : 'new-password'} aria-invalid={Boolean(error)} aria-describedby={error ? `${name}-error` : undefined} className={inputClass} {...register(name, rules)} />{error && <span id={`${name}-error`} role="alert" className="mt-2 block text-xs font-medium text-red-500">{error.message}</span>}</label>; }
...
function DeactivateModal({ busy, error, onClose, onConfirm }) { useEffect(() => { const close = (event) => event.key === 'Escape' && !busy && onClose(); window.addEventListener('keydown', close); return () => window.removeEventListener('keydown', close); }, [busy, onClose]); return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="deactivate-title" onMouseDown={onClose}><motion.div initial={{ opacity: 0, y: 18, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.97 }} onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-lg rounded-[2rem] border border-red-500/20 bg-[var(--bg-surface)] p-6 shadow-[var(--shadow-elevated)] sm:p-7"><div className="flex items-start justify-between gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-500/10 text-red-500"><AlertTriangle className="h-6 w-6" /></span><button type="button" onClick={onClose} disabled={busy} className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--border)] text-[var(--text-muted)] disabled:opacity-40" aria-label="Close confirmation"><X className="h-4 w-4" /></button></div><h2 id="deactivate-title" className="mt-5 text-2xl font-semibold text-[var(--text-primary)]">Leave as Chef?</h2><p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">HomeBite will first check that you have no active orders. If clear, your account becomes a customer account and all non-deleted foods become inactive.</p>{error && <div role="alert" className="mt-4 rounded-2xl border border-red-500/20 bg-red-500/5 p-4 text-sm font-medium text-red-500">{error}</div>}<div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} disabled={busy} className="rounded-full border border-[var(--border)] px-5 py-3 text-sm font-semibold text-[var(--text-secondary)] disabled:opacity-40">Cancel</button><button type="button" onClick={onConfirm} disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-full bg-red-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-red-500/20 disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldOff className="h-4 w-4" />}{busy ? 'Checking Orders...' : 'Confirm & Leave'}</button></div></motion.div></motion.div>; }
